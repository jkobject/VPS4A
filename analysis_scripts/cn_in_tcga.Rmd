...
plot_saving_directory <- '~/Documents/gkugener/VPS4A/plots/cn_in_tcga/'
dir.create(plot_saving_directory)
```

```{r eval=FALSE}
# To push this to CDS_Projects
cds_render(path_to_rmd = '~/Documents/gkugener/VPS4A/cn_in_tcga.Rmd', folder = 'VPS4A', path_to_html = '~/Documents/gkugener/VPS4A/cn_in_tcga.html')
```
...
vps4a_b_only %>%
  ...
  write.table(., file = '~/Documents/Analysis/VPS4A/tables/tcga_vps4b_copy_number.tsv', sep = '\t', quote = F, row.names = F)

vps4a_b_only %>%
  filter(`Gene Symbol`=='VPS4A') %>%
  dplyr::rename(relative_cn=CN) %>%
  write.table(., file = '~/Documents/Analysis/VPS4A/tables/tcga_vps4a_copy_number.tsv', sep = '\t', quote = F, row.names = F)
...
vps4a_b_only %>%
  ...
  write.table(., file = '~/Documents/Analysis/VPS4A/tables/vps4ab_copy_number_by_cancer_type.tsv', sep = '\t', quote = F, row.names = F)
```
...
hg29_ids <- read_csv('~/Documents/Analysis/RScripts/Common_annotations/v29_ensembl_hgnc_gene_names.csv') %>%
  filter(hgnc_id %in% gsub(' .*', '', colnames(public.CCLE.expression))) %>%
...
tcga_genes_chosen <- sample(common_genes_between_tcga_datatsets, size=4000)
tcga_genes_chosen <- c(tcga_genes_chosen, c("VPS4A", "VPS4B")) %>% unique()
